 ...
 ************************************************
 ...
 //////////////////////////////////////////////////////////////
 //////// MaKey MaKey FIRMWARE v1.4.2 (modified!) /////////////
 //////////////////////////////////////////////////////////////
 by: Eric Rosenbaum, Jay Silver, and Jim Lindblom
 MIT Media Lab & Sparkfun
 start date: 2/16/2012 
 release of original version: 7/5/2012

 modified to read key configuration from EEPROM
 modifications by: maehw
 date of modifications: 25/5/2020
 */
...
void initializeInputs() {
  ...
  for (int i=0; i<NUM_INPUTS; i++) {
    ...
    if(useEepromConfig)
    {
      EEPROM.get(2*i, inputs[i].keyCode);
    }
    else
    {
      inputs[i].keyCode = defaultKeyCodes[i];
    }
...
void checkEepromUsage(void)
{
  int testVal = 0;
  EEPROM.get(0, testVal);
  useEepromConfig = ( testVal != -1 ); /* checks against blank EEPROM, 0xFFFF = -1 */
}
